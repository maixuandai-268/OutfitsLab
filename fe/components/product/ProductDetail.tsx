...
export const ProductDetail = ({ product }: ProductDetailProps) => {
  ...
  const [selectedColor, setSelectedColor] = useState<string | null>(null);
	const filteredProducts = PRODUCTS.filter(p => p.shop_id === product.shop_id && p.id !== product.id);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8"> 
			 <div className="w-full md:w-1/2 px-4"> {/*Ảnh sản phẩm */}
					<img src={product.image_url} alt={product.name} className="rounded-lg min-h-150 object-fit shadow-4xl"/>
					<div>
						<p className="font-semibold mt-6">Best mix style with</p>
						<div className="flex gap-4 mt-4">
							{PRODUCTS.slice(0, 3).map(item => (
								<div key={item.id} className="relative flex-1 border border-gray-400 rounded-b-xl pb-14">
									<div>
										<img src={item.image_url} alt={item.name} className="w-full h-40 object-cover border-b border-gray-400"/>
									...
									<div className="absolute bottom-0 w-full">
										...
										<div className="flex justify-between mb-1 m-1">
											<p className="flex-1 text-xs text-gray-500">{product.sold} Sold</p>
			...
			<div className="w-full md:w-1/2 px-4"> {/*Thông tin sản phẩm */}
				...
				<p className="text-3xl mb-6 font-bold text-[#d19f42]">${product.price}</p>
					...
					<div className="grid grid-cols-6 md:grid-cols-8 gap-2">
						{colors.map(item => (
							<button
								key={item.color}
								type="button"
								className={`w-10 h-10 rounded-full border-2 flex justify-center items-center ${selectedColor === item.color ? 'border-[#d19f42] scale-120' : 'border-gray-300 hover:border-[#d19f42]'}`}
					...
					<div className="grid grid-cols-3 md:grid-cols-6 gap-2">
						{sizes.map(size => (
							<button
								key={size}
								type="button"
								className={`w-full h-12 rounded-lg font-semibold border-2 transition-colors duration-200 ${selectedSize === size ? 'bg-[#d19f42] text-white border-[#d19f42]' : 'border-[#ffe9cc] hover:bg-[#fff4e6]'}`}
				...
				<div className="flex gap-4 pt-8">
					<button
						type="button"
						className="px-6 py-3 bg-[#d19f42] text-white font-bold rounded-full flex items-center justify-center hover:bg-[#b88a36] transition-colors duration-200 shadow-md hover:shadow-lg"
					>
					...
					<button
						type="button"
						className="group px-4 py-3 border-2 border-[#d19f42] text-[#d19f42] font-bold rounded-full hover:bg-orange-50 flex justify-center items-center transition-colors duration-200"
