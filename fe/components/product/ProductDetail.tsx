  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      <div className="w-full md:w-1/2 px-4"> {/*Ảnh sản phẩm */}
        <img src={product.image || product.image_url || 'https://via.placeholder.com/500'} alt={product.name} className="rounded-lg w-full min-h-150 max-h-150 object-cover shadow-4xl" />
      ...
      <div className="w-full md:w-1/2 px-4"> {/*Thông tin sản phẩm */}
        ...
        <div className="flex items-center gap-4 mb-5">
          <p className="text-base text-gray-500">{product.salesCount || product.salesCount || 0} Đã bán</p>
          <div className="flex items-center gap-1">
            <StarIcon className="text-yellow-500 w-5 h-5" filled={true} />
            <p className="text-sm">{product.rating || 5} ({product.reviews || 0} Đánh giá)</p>
          </div>
        </div>
        ...
        <div className="flex gap-4 pt-8 shrink-0">
          <button
            type="button"
            className="px-6 py-3 bg-[#fa649a] text-white font-bold rounded-full flex items-center justify-center hover:bg-[#f52873] transition-colors duration-200 shadow-md hover:shadow-lg flex-1"
          >
            <MagicWand className="w-5 h-5 mr-2 shrink-0" />
            Thử đồ ngay!
          </button>

          <div className="w-16 h-[52px]">
            <FavoriteButton
              targetId={product.id}
              className="!w-full !h-full border-2 border-gray-200 !bg-transparent hover:!bg-rose-50 rounded-full flex items-center justify-center [&_svg]:!w-6 [&_svg]:!h-6"
            />
          </div>
        </div>
