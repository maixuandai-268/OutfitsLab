...
export default function Navbar() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "https://outfitslab.onrender.com/api/notifications",
        {
  ...
  const handleMarkAsRead = async (id: number) => {
    try {
      const res = await fetch(
        `https://outfitslab.onrender.com/api/notifications/${id}/read`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const res = await fetch(
        `https://outfitslab.onrender.com/api/notifications/read-all`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    if (user?.role === "shop") {
      baseItems.push(
        { type: "divider" },
        {
          ...
          icon: <ShopOutlined />,
          label: (
            <span onClick={handleShopClick} className="cursor-pointer">
              Chi tiết cửa hàng
            </span>
          ),
        },
    ...
    <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-80 overflow-hidden">
      ...
      <div className="p-2 border-t border-gray-50 text-center">
        <Link
          ...
          className="text-[12px] font-bold text-gray-400 hover:text-blue-500 transition-colors"
        >
